---
title: "One-way sensitivity analysis - pre-calibration for status quo model - Ontario"
format:
  html:
    toc: true
...
t_owsa_ini_pop <- read.csv(file = here("01_data/ontario/ows_tbl_ini_pop_on.csv")) %>% select(-X)
ows_tbl_prob <- read.csv(, file = here("01_data/ontario/ows_tbl_prob_on.csv")) %>% select(-X)
...
t_owsa_ini_pop_costs <- t_owsa_ini_pop_costs %>% 
  ...
  filter(costs_new != 0) %>% 
  mutate(group = ifelse(group == "costs_high", "> basecase value",
                        ifelse(group == "costs_low", "< basecase value", group)))


ggplotly(ggplot() +
  geom_bar(data = t_owsa_ini_pop_costs %>% filter(abs(costs_new) >= 50000000),
...
t_owsa_ini_pop_deaths <- t_owsa_ini_pop_deaths %>% 
  ...
  mutate(deaths_new = (deaths - mod_basecase$m_M[n_cycles+1, "BO_DEATH"])) %>% 
  filter(deaths_new != 0) %>% 
  mutate(group = ifelse(group == "deaths_high", "> basecase value",
                        ifelse(group == "deaths_low", "< basecase value", group)))

ggplotly(ggplot() +
  geom_bar(data = t_owsa_ini_pop_deaths  %>% filter(abs(deaths_new) >= 500),
  ...
  labs(color = "Difference in initial \n population value", fill = "Difference in initial \n population value") +
    scale_x_discrete(labels=c("ini_pop_BN_CHRONIC" = "Chronic pain, no use",
                              ...
                              "ini_pop_BN_PALLIATIVE" = "Palliative, no use",
                              "ini_pop_BS_OAT_MAINT" = "OAT maintenance")) +
    scale_color_brewer(palette = "Dark2") +
    scale_fill_brewer(palette = "Dark2"))
```
...
t_owsa_ini_pop_oddeaths <- t_owsa_ini_pop_oddeaths %>% 
  ...
  mutate(oddeaths_new = (oddeaths - (mod_basecase$m_M[n_cycles+1, "BO_OD_DEATH"] + mod_basecase$extra_od_deaths))) %>% 
  filter(oddeaths_new != 0) %>% 
  mutate(group = ifelse(group == "od_deaths_high", "> basecase value",
                        ifelse(group == "od_deaths_low", "< basecase value", group)))

ggplotly(ggplot() +
  geom_bar(data = t_owsa_ini_pop_oddeaths %>% filter(abs(oddeaths_new) >= 25),
...
ows_tbl_prob_deaths <- ows_tbl_prob_deaths %>% 
  pivot_longer(1:2, names_to = "group", values_to = "deaths") %>% 
  mutate(deaths_new = (deaths - as.numeric(mod_basecase$m_M[n_cycles + 1, "BO_DEATH"])),
         deaths_new_per = (deaths_new/as.numeric(mod_basecase$m_M[n_cycles + 1, "BO_DEATH"])) * 100) %>% 
  filter(deaths_new != 0) %>% 
  mutate(group = ifelse(group == "deaths_high", "> basecase value",
                        ifelse(group == "deaths_low", "< basecase value", group)))
...
oddeaths_bc <- as.numeric(mod_basecase$m_M[n_cycles+1, "BO_OD_DEATH"] + mod_basecase$extra_od_deaths) 
