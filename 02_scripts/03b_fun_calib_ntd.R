...
ows_tbl_prob_ntd <- read.csv(here("01_data/ows_tbl_prob_ntd.csv"))
...
deaths_bc_ntd <- mod_basecase_notimedep$m_M[181, "BO_DEATH"]
od_deaths_bc_ntd <- mod_basecase_notimedep$m_M[181, "BO_OD_DEATH"] + mod_basecase_notimedep$extra_od_deaths

# subset parameters form ows_tbl_prob that resulted in more than 2.5% difference in deaths and od deaths
ows_tbl_prob_subset5_ntd <- ows_tbl_prob_ntd %>%
  filter((abs(deaths_bc_ntd - deaths_low) / deaths_bc_ntd) > 0.025 |
           (abs(deaths_high - deaths_bc_ntd) / deaths_bc_ntd) > 0.025 |
           (abs(od_deaths_bc_ntd - od_deaths_low) / od_deaths_bc_ntd) > 0.025 |
           (abs(od_deaths_high - od_deaths_bc_ntd) / od_deaths_bc_ntd) > 0.025) %>% 
...
calib_param_tbl_org_ntd <- as.data.frame(m_P) %>% 
...
calib_param_tbl_ntd <- ows_tbl_prob_subset5_ntd %>% 
  bind_rows(., calib_param_tbl_org_ntd %>% 
              select(var_params) %>% 
              filter(grepl("DEATH", var_params))) %>% 
  left_join(., calib_param_tbl_org_ntd, by = "var_params") %>% 
  arrange(var_params) %>% 
...
v_params_to_be_calib_ntd <- calib_param_tbl_ntd$var_params_cycle
n_params_ntd <- length(v_params_to_be_calib_ntd)
...
mod_calib_ntd <- function(v_parms_calib) {
  
  v_parms_calib <- suppressWarnings({
    data.frame(v_parms_calib) %>% 
      mutate(params = v_params_to_be_calib_ntd) %>% 
